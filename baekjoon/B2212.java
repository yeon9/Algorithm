...
집중국의 수신 가능 영역은 고속도로 상에서 연결된 구간으로 나타나게 된다. N개의 센서가 적어도 하나의 집중국과는 통신이 가능해야 하며, 
...
public class B2212 {
    public static void main(String[] args) {
        ...
        for(int i=0; i<n; i++){     // 센서 거리 입력 받음
            sensor[i] = sc.nextInt();
        }

        if(n <= k){     // 집중국의 수가 센서 수보다 많은 경우
            System.out.println(0);
            return;
        } 

        Arrays.sort(sensor);    // 센서들 거리 순으로 정렬

        for(int i=0; i<n-1; i++){
            diff[i] = sensor[i+1]-sensor[i];  // 센서 끼리의 거리 배열에 저장    
        }

        // diff 배열을 오름차순으로 정렬
        Integer[] diffI = Arrays.stream(diff).boxed().toArray(Integer[]::new);
        Arrays.sort(diffI, Collections.reverseOrder());
